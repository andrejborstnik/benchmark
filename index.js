...
function runSuites(suites) {
  ...
  function runSuite(suiteName, tests) {
    return new Promise((resolve) => {
      ...
      const getRunName = (testName, projectName) =>
      ...
      testNames.forEach((testName) => {
        Object.keys(tests[testName]).forEach((projectName) => {
          ...
          suite.add(
            getRunName(testName, projectName),
      ...
      const projectNamesWithVersions = projectNames.map(
        ...
        (lib) =>
          `${lib} (v${
            require(`${
              lib === 'styled' ? 'styled-components' : lib
      ...
      suite.on(
        'error',
        (err) => console.error(err.target.error) && suite.abort()
      )
        ...
        const rows = testNames.map((testName) =>
          [testName].concat(
            projectNames.map((projectName) => {
              ...
              return run
                ? `${(run.stats.mean * 1000).toFixed(3)}±${(
                    run.stats.deviation * 1000
                  ).toFixed(3)}ms`
                : '-'
            })
          )
  ...
  const results = Object.keys(suites).map((suiteName) =>
    runSuite(suiteName, suites[suiteName])
  )
